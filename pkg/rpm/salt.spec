...
#Source11: salt.bash
Source12: %{name}-maid.service
Source13:  %{name}-apiv2
Source14: %{name}-apiv2.service
...
Requires: python26-redis
Requires: numpy
Requires: python26-tornado
...
%description maid
The Salt maid is a master daemon which can receive instruction from a
higher-level master, allowing for tiered organization of your Salt
infrastructure.
...
%package apiv2
Summary: REST API V2 for Salt, a parallel remote execution system
Group:   System administration tools
Requires: %{name}-master = %{version}-%{release}
%if 0%{?with_python26}
Requires: python26-cherrypy
...
%description apiv2
salt-apiv2 provides a REST interface to the Salt master.
...
%doc %{_mandir}/man1/saltx.1.gz
%doc %{_mandir}/man1/salt-maid.1.gz
%doc %{_mandir}/man1/salt-apiv2.1.gz
%{python_sitelib}/%{name}/*
...
%files maid
%doc %{_mandir}/man1/salt-maid.1.*
%{_bindir}/salt-maid
%if ! (0%{?rhel} >= 7 || 0%{?fedora} >= 15)
%attr(0755, root, root) %{_initrddir}/salt-maid
%else
%{_unitdir}/salt-maid.service
%endif
...
%files apiv2
%defattr(-,root,root)
%doc %{_mandir}/man1/salt-apiv2.1.*
%{_bindir}/salt-apiv2
%if ! (0%{?rhel} >= 7 || 0%{?fedora} >= 15)
%attr(0755, root, root) %{_initrddir}/salt-apiv2
%else
%{_unitdir}/salt-apiv2.service
%endif
