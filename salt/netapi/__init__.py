...
class NetapiClient(object):
    ...
    def apiv2_jobs(self, fun, timeout=None, full_return=False, **kwargs):
        ...
        try:
            ...
            _redis_type = salt_config.get('redis_type')

            _cluster_redis_host = salt_config.get('cluster_redis_host')
            _cluster_redis_port = salt_config.get('cluster_redis_port')
            _cluster_redis_password = salt_config.get('cluster_redis_password')

            sub_timeout = salt_config.get('channel_sub_timeout')
            sub_node = ''
            _channel_redis_sentinel = salt_config.get('channel_redis_sentinel')
            _channel_redis_password = salt_config.get('channel_redis_password')
            _master_pub_topic = salt_config.get('id')
            ...
            self.bootConfig = {'_sub_timeout': sub_timeout, '_sub_node': sub_node,
                               ...
                               '_redis_type': _redis_type,
                               '_cluster_redis_host': _cluster_redis_host,
                               '_cluster_redis_port': _cluster_redis_port,
                               '_cluster_redis_password': _cluster_redis_password
                               }
            ...
            while True:
                try:
                    ...
                    if message:
                        ...
                        api_log.info('baba: %s, %s, %s, %s, %s' % (syndic_count, resultCount, pingCount, runningCount,
                                                             [i for i in comeSubList if i not in resultExeSet]))

                        # if pingCount < syndic_count and runningCount <= 0:
                        #     if (time.time() - executeStart) > 5:
                        #         break
                        api_log.info("time: %s" % (time.time() - executeStart))
                    if (time.time() - executeStart) > 5:
                        break
    ...
    def getPassedIp(self):
        ...
        # except Exception:
        #     api_log.info("==== ignore /data0/md/ip.md")
        #     return []
