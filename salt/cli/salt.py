...
from salt.selflog.logger_util import logger
...
class SaltCMD(parsers.SaltCMDOptionParser):
    ...
    def new_run(self):
        ...
        sub_timeout = self.newopt.get('channel_sub_timeout')
        ...
        if isinstance(kwargs['tgt'], list):
            ...
            if isinstance(passed_ip, str):
                passed_ip = [passed_ip]
            kwargs['tgt'] = list(set(kwargs['tgt']).difference(set(passed_ip)))

            timeoutSet = set(kwargs['tgt'])
            if len(kwargs['tgt']) == 0:
        ...
        if self.config['async']:
            ...
            # NOTE 执行节点填充
            def nodeStartExchange():
                while len(batch_running) < batch_hold:
                    try:
                        popInit = batch_init.pop()
                        batch_wait.add(popInit)
                        batch_running.add(popInit)
                    except:
                        break
                return True

            def batchExecuteCallback(selfIp, clientPub, redisChannel):
                if nodeStartExchange() and len(batch_wait) > 0:
                    # trigger to sub run
                    tmpKwargs = wrapMesage['kwargs']
                    tmpKwargs['tgt'] = list(batch_wait)
                    wrapMesage['kwargs'] = tmpKwargs

                    wrapSubMesage = wrapMesage
                    batchRun(wrapSubMesage, selfIp, clientPub, redisChannel)
            ...
            def batchRun(wrapMesage, selfIp, clientPub, redisChannel):
                ...
                global batch_running
                batch_running = batch_running.union(set(tmpKwargs1['tgt']))
                ...
                for message in redisChannel.listen():
                    try:
                                ...
                                if isinstance(callResult, dict):
                                    if 'type' in callResult:
                                        ...
                                        if messageType == MessageType.PING and messageIp in comeSubList:
                                            ...
                                            logger.info('invalid callresult: %s' % callResult)
                                        ...
                                        if retJsonObj:
                                            ...
                                            if tmpRunCount == batch_hold and iCount != rCount:
                                                time.sleep(bwait)
                                                # reset the count looper
                                                tmpRunCount = 0

                                            if len(batch_running) < batch_hold:
                                                try:
                                                    popInit = batch_init.pop()
                                                    batch_wait.add(popInit)
                                                    batch_running.add(popInit)

                                                    tmpKwargs2 = wrapMesage['kwargs']
                                                    tmpKwargs2['tgt'] = list(batch_wait)
                                                    wrapMesage['kwargs'] = tmpKwargs2
                                                    batch_running = batch_running.union(set(batch_wait))
                        ...
                        #log.info("%s, %s, %s, %s" % (pingCount, resultCount, len(resultPingSet), len(resultExeSet)))
                        # if len(batch_init) <= 0:
                        #     break
            ...
            def executeCallback(selfIp):
                ...
                #for message in redisChannel.listen():
                    try:
                                ...
                                try:
                                    ...
                                    if isinstance(callResult, dict):
                                        if 'type' in callResult:
                                            ...
                                            if retJsonObj:
                                                # reset start time
                                                #logger.info(json.dumps(retJsonObj))
                                                if callResult['out'] == 'no_return':
                                                    if '[No response]' in json.dumps(retJsonObj):
                                                        noResponseRet.append(callResult)
                                                    else:
                        ...
                        #noResultSet = [item for item in comeSubList if item not in resultExeSet]
                        #logger.info("%s,%s,%s,%s,%s,%s,%s" % (pingCount, syndic_count, runningCount, sub_timeout, executeStart, lossPing, noResultSet))
                        ...
                        #logger.info("c1: %s, c2: %s" % ((time.time() - executeStart), sub_timeout))
                        if syndic_count == pingCount and runningCount > 0:
                            if (time.time() - executeStart) > sub_timeout:
                                # main_log.info("---T1 stop")
                                break

                        if syndic_count == pingCount and resultCount == syndic_count:
