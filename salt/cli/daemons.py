...
class SaltMaid(parsers.SyndicOptionParser, DaemonsMixin):
    '''
    Create a sub server
    '''
    ...
    def start(self):
        ...
        NOTE: Run any required code before calling `super()`.
        '''
        super(SaltMaid, self).start()

        self.parse_args()

        self.bootConfig = {'_sub_node': self.config.get('id'),
                           '_channel_redis_sentinel': self.config.get('channel_redis_sentinel'),
                           '_channel_redis_password': self.config.get('channel_redis_password'),
                           '_master_pub_topic': self.config.get('super_master'),
                           '_redis_type': self.config.get('redis_type'),
                           '_cluster_redis_host': self.config.get('cluster_redis_host'),
                           '_cluster_redis_port': self.config.get('cluster_redis_port'),
                           '_cluster_redis_password': self.config.get('cluster_redis_password'),
                           }
        #print("bootConfig is %s" % self.bootConfig)
        #log.info("Using bootConfig: %s" % self.bootConfig)

        #print('self.opts: %s' % self.config)
        import salt.newrun
        sub = salt.newrun.SyndicSubcribe(**self.bootConfig)
        sub.startListen(self.config)
    ...
    def shutdown(self, exitcode=0, exitmsg=None):
        ...
        '''
        self.action_log_info('Shutting down')
        super(SaltMaid, self).shutdown(
            exitcode, ('The Salt {0} is shutdown. {1}'.format(
                self.__class__.__name__, (exitmsg or '')).strip()))

class SaltAPIV2(parsers.SaltAPIParser):
    '''
    Create a api server
    '''
    ...
    def start(self):
        ...
        NOTE: Run any required code before calling `super()`.
        '''
        super(SaltAPIV2, self).start()

        self.daemonize_if_required()
        self.set_pidfile()

        import salt.apiv2
        api = salt.apiv2.SaltApiV2()
        api.run()
    ...
    def shutdown(self, exitcode=0, exitmsg=None):
        ...
        :param exitmsg
        '''
        #self.action_log_info('Shutting down api')
        super(SaltAPIV2, self).shutdown(
            exitcode, ('The Salt {0} is shutdown. {1}'.format(
                self.__class__.__name__, (exitmsg or '')).strip()))
