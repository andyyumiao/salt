...
logger = ManualLog().get_logger('salt_api_module')
...
try:
    from salt.netapi.rest_cherrypy.tools import websockets
    from salt.netapi.rest_cherrypy import event_processor
...
class LowDataAdapter(object):
    ...
    def exec_lowstate(self, client=None, token=None):
        print('Pls note: this is the The primary entry point to Salt REST API.')
        '''
        ...
        api_log.info('----LowDataAdapter-----get-------referer------:%s' % referer)
        ...
        for chunk in lowstate:
            ...
            api_log.info('----LowDataAdapter-----apiv2_run------:%s,%s' % (cherrypy.request.headers.get('Remote-Addr', 'unknown host').lower(), chunk))
            ret = self.api.apiv2_run(chunk)
            # api_log.info('----LowDataAdapter-----apiv2_run_result------:%s' % ret)
    ...
    def exec_lowstate_jobs(self, client=None, token=None):
        print('Pls note: this is the The primary entry point to Salt REST API.')
        '''
        Pull a Low State data structure from request and execute the low-data
        chunks through Salt. The low-data chunks will be updated to include the
        ...
        for chunk in lowstate:
            ...
            api_log.info('----LowDataAdapter-----run_jobs------:%s, %s' % (cherrypy.request.headers.get('Remote-Addr', 'unknown host').lower(), chunk))
            ret = self.api.apiv2_run_jobs(chunk)
            return ret

    def exec_lowstate_aysnc(self, client=None, token=None):
        logger.info('Pls note: this is the The primary entry point to Salt REST API.')
        '''
        Pull a Low State data structure from request and execute the low-data
        chunks through Salt. The low-data chunks will be updated to include the
        authorization token for the current session.
        ...
        for chunk in lowstate:
            ...
            api_log.info('----LowDataAdapter-----apiv2_aysnc_run------:%s, %s' % (cherrypy.request.headers.get('Remote-Addr', 'unknown host').lower(), chunk))
            ret = self.api.apiv2_aysnc_run(chunk)
...
class Minions(LowDataAdapter):
    ...
    def GET(self, mid=None):
        ...
        **Example response:**
        ...
        job_data = self.exec_lowstate_aysnc(client='local_async',
            token=cherrypy.session.get('token'))

        return {
            'return': job_data['return'],
        }
...
class Jobs(LowDataAdapter):
    ...
    def GET(self, jid=None, timeout=''):
        ...
        **Example response:**
        ...
        '''
        lowstate = {'client': 'runner', 'jid': jid}
        if jid:
        ...
        #job_ret_info = list(self.exec_lowstate_jobs(token=cherrypy.session.get('token')))
        job_ret_info = self.exec_lowstate_jobs(token=cherrypy.session.get('token'))
    ...
    def get_conf(self):
        ...
        conf = {
            'global': {
                ...
                'server.socket_port': self.apiopts.get('port', 8088),
                # 'server.thread_pool': self.apiopts.get('thread_pool', 20),
